...
public class GameManagerRemake : MonoBehaviour
{
    ...
    public BattleManagerRemake BattleMan;

    [Header("Classes & Encounter")]
    public List<ClassPlayerRemake> AllClasses;

    public List<EncounterRemake> AllEncounter;

    public ClassPlayerRemake classSO;
    ...
    private void LoadSave()
    {
        ...
        if (File.Exists(path))
        {
            ...
            {
                getClassRun();
                playerStat = new JoueurStatRemake()
                {
                    Radiance = loadedData.CurrentRun.player.HP,
                    RadianceMax = classSO.PlayerStat.RadianceMax,
                    Volonter = loadedData.CurrentRun.player.Volonté,
                    VolonterMax = classSO.PlayerStat.VolonterMax,
    ...
    private void CreateSave()
    {
        ...
        var spellsToAdd = AllClasses.First(c => c.ID == 0).spellClass.Where(c => c.SpellStatue == SpellStatusRemake.bought);
        ...
        {
            HP = AllClasses.First(c => c.ID == 0).PlayerStat.Radiance,
            Conscience = AllClasses.First(c => c.ID == 0).PlayerStat.Conscience,
            dmg = AllClasses.First(c => c.ID == 0).PlayerStat.ForceAme,
            Speed = AllClasses.First(c => c.ID == 0).PlayerStat.Vitesse,
            Volonté = AllClasses.First(c => c.ID == 0).PlayerStat.Volonter,
    ...
    public void SaveGame()
    {
        ...
        if (loadedData.CurrentRun.Ended)
        {
            ...
            {
                HP = classSO.PlayerStat.Radiance,
                Volonté = classSO.PlayerStat.Volonter,
                Conscience = classSO.PlayerStat.Conscience,
                Essence = classSO.PlayerStat.Essence,
                dmg = classSO.PlayerStat.ForceAme,
                Speed = classSO.PlayerStat.Vitesse,
    ...
    private void SavePlayer()
    {
        loadedData.CurrentRun.player = new PlayerData()
        {
            HP = playerStat.Radiance,
            Volonté = playerStat.Volonter,
            Conscience = playerStat.Conscience,
            Essence = playerStat.Essence,
            dmg = playerStat.ForceAme,
            Speed = playerStat.Vitesse
        };
