...
public class JoueurBehavior : CombatBehavior
{
    ...
    void Dead()
    {
        GameManagerRemake.instance.BattleMan.DeadPlayer();
    }
    ...
    {
        GameManagerRemake.instance.BattleMan.StartTargeting();
    }
    ...
    public void SendSpell()
    {
        ...
        GameManagerRemake.instance.BattleMan.LaunchAnimAttacked();
    }

    private void AfterAnim()
    {
        //A Mettre une fois les combats terminer
        GameManagerRemake.instance.BattleMan.LaunchSpellJoueur(SelectedSpell);
        ActivateSpells();
        UpdateUI();
    }
    ...
    public void ApplicationBuffDebuff(TimerApplication Timer)
    {
        ...
        foreach (var item in Stat.ListBuffDebuff)
        {
            if(item.Activate == true && (item.timerApplication == Timer || item.timerApplication == TimerApplication.Persistant))
            {
                ...
                {
                    GameManagerRemake.instance.BattleMan.PassageEffet(effet, item.IDCombatOrigine, 0);
    ...
    public void ApplicationEffet(EffetRemake effet, EnnemiStatRemake Caster = null)
    {
