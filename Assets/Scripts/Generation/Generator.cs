...
public class Generator : MonoBehaviour
{
    ...
    [Header("Spawned Object")] public Dictionary<Vector2, GameObject> spawnedRoomsObj;

    [Header("Spawnable")] [SerializeField] private GameObject roomPrefab;
    [SerializeField] private GameObject pathPrefab;


    [Header("Rooms Display Settings")] [SerializeField]
    private Vector2 mapAreaSize;
    ...
    [SerializeField]
    private List<TypeRoom> roomPool = new List<TypeRoom>();

    [SerializeField] private int defaultRowSize;
    [SerializeField] private int defaultColSize;

    [SerializeField, Tooltip("0 for random")]
    private int seed;

    [Header("DEBUG")] [SerializeField] private bool useLoad = false;
    [SerializeField] private bool doCycle = false;
    [SerializeField] private float cycleTime = 1f;

    [Header("Forbiden connections")] [SerializeField]
    private List<TypeRoom> illegalConnection_START = new List<TypeRoom>();
    ...
    private List<TypeRoom> availableRoomPool = new List<TypeRoom>();
    ...
    public void GenerateNewMap()
    {
        ...
        int elitCnt = availableRoomPool.Where(rType => rType == TypeRoom.ELITE || rType == TypeRoom.CLASS_ELITE)
            .Count();
        int defaultRoomCnt = 2 /*Start & Boss*/ + availableRoomPool.Count(); //Loots rooms not counted
    ...
    private void GenerateMap(int roomCnt)
    {
        ...
        List<TypeRoom> firstOptions = availableRoomPool.Where(rType => validFirstRooms.Contains(rType)).ToList();
        ...
        availableRoomPool.Remove(mapNodes[1].roomType);
        availableRoomPool.Remove(mapNodes[2].roomType);

        mapNodes[roomCnt - 2].roomType = availableRoomPool[Random.Range(0, availableRoomPool.Count() - 1)];
        mapNodes[roomCnt - 2].connections.Add(roomCnt - 1);
        HandleLootRooms(mapNodes[roomCnt - 2].roomType, roomCnt - 2);
        availableRoomPool.Remove(mapNodes[roomCnt - 2].roomType);

        mapNodes[roomCnt - 3].roomType = availableRoomPool[Random.Range(0, availableRoomPool.Count() - 1)];
        mapNodes[roomCnt - 3].connections.Add(roomCnt - 1);
        HandleLootRooms(mapNodes[roomCnt - 3].roomType, roomCnt - 3);

        availableRoomPool.Remove(mapNodes[roomCnt - 3].roomType);

        //Choose Types And Connections

        //FILL UP AVIABLES POOL WITH NONE
        if (availableRoomPool.Count() < roomCnt - 6)
        {
            for (int i = availableRoomPool.Count(); i < (roomCnt - 6); i++)
            {
                availableRoomPool.Add(TypeRoom.NONE);
            }
        }
        ...
        for (int loopId = 0; loopId < maxLoop; loopId++)
        {
            ...
            for (int i = 0; i < roomCnt - 6; i++)
            {
                ...
                List<TypeRoom> validRooms = GetPossibleRoomTypes(nonEmptyconnectedRooms, availableRoomPool);

                if (validRooms.Count() <= 0)
                {
                    //Debug.Log($"/!\\\nno aviable room choice for room at pos {roomIndex}");
                    //Debug.Log("choice Remaining:");
                    //foreach (TypeRoom room in availableRoomPool)
                    //{
                    //    Debug.Log(room);
                    //}
                ...
                availableRoomPool.Remove(validRooms[choosedRoomId]);

            }

            if (availableRoomPool.Count() > 0)
            {
                //Debug.Log("room Left to Pick:");
                //foreach (TypeRoom type in availableRoomPool)
                //{
            ...
            foreach (int source in interco)
            {
                foreach (int dest in interco)
                {
                    ...
                    if (GetPossibleRoomTypes(
                        ...
                        && !((source == roomCnt - 2 || source == roomCnt - 3) &&
                             (dest == roomCnt - 2 || dest == roomCnt - 3))
                       )
                    {
    ...
    private void LengthBasedDecimatePath(int roomCnt)
    {
        for (int i = 0; i < roomCnt; i++)
        {
            ...
            connectionLength = connectionLength.OrderByDescending(kvPair => kvPair.Value)
    ...
    private bool doPathCross(Vector2 firstPathStart, Vector2 firstPathEnd, Vector2 secondPathStart,
        Vector2 secondPathEnd)
    {
        ...
        for (int i = 0; i < roomCnt; i++)
        {
            ...
            for (int j = 0; j < cntToRemove; j++)
            {
                ...
                for (int k = 0; k < mapNodes[i].connections.Count(); k++)
                {
                    ...
                    if (mapNodes[connectedRoomId].connections.Count() > 2 && connectedRoomId != 0 &&
                        connectedRoomId != roomCnt - 1)
                    {
    ...
    private List<int> GetPossibleConnections(int nodeIndex, int roomCnt)
    {
        ...
        if (nodeIndex < 3 || nodeIndex > roomCnt - 4)
        {
            ...
            {
                for (int i = roomCnt - 3 - defaultRowSize;
                     i < (roomCnt - 3) - Mathf.FloorToInt(defaultRowSize / 2f);
                     i++)
                {
    ...
    {
        int defaultRoomCnt = 2 /*Start & Boss*/ + availableRoomPool.Count();
        int rowCnt =
            Mathf.Max(defaultColSize,
                Mathf.FloorToInt((defaultRoomCnt - 6) / defaultRowSize) +
                1); //Mathf.FloorToInt((float)roomPool.Count() / (float)defaultRowSize);//last row will overflow
