    ...
    private string ApplyTag(TradTag tag, Dictionary<TradAttribute, string> attributes)
    {
        ...
        switch (tag)
        {
            ...
            case TradTag.stat:
                ...
                else if (attributes[TradAttribute.value].Equals("DEF", System.StringComparison.InvariantCultureIgnoreCase))
                    strb.Append((_clairvoyanceIconData.IncreaseDef == null) ? "ATKM" : _clairvoyanceIconData.IncreaseDef.name);
                else
            ...
            case TradTag.percent:
                ...
                else if (attributes[TradAttribute.target].Equals("RAD", System.StringComparison.InvariantCultureIgnoreCase))
                {
                    if (GameManager.Instance == null && TutoManager.Instance != null)
                    {
                        numericValue = GameManager.Instance.playerStatHandler.Radiance * (value / 100f);
                    }
                    else if (GameManager.Instance.playerStatHandler != null)
                    {
                        numericValue = GameManager.Instance.playerStatHandler.Radiance * (value / 100f);
